...
# Hiperparâmetros
STATE_SIZE = 1      # estado (ex.: erro lateral)
ACTION_SIZE = 1     # saída contínua (ex.: velocidade angular)
ACTION_LIMIT = 1.0  # limite da ação


# Rede Actor (igual à usada no treino)
class Actor(nn.Module):
    ...
    def forward(self, x):
        return self.fc(x) * ACTION_LIMIT  # escala para o limite definido
...
def timer_callback():
    ...
    # Estado usado pelo ator (DDPG)
    state = np.array([error])

    vel_angular = select_action(state)
    message.angular.z = float(vel_angular)
    ...
    # Escreve informações diretamente na imagem
    cv2.putText(
        output,
        f"Erro: {error:.2f}",
        (10, 30),
...
def select_action(state):
    # Seleciona uma ação contínua usando a rede Actor
